...
data TargetModel = TargetModel
                   ...
                   , tmodelProcs :: Map String (GBuchi (Integer,Int) BuchiVariables Bool)
      ...
      let mdl = (gtlSpecModels spec)!(gtlInstanceModel inst)
          ...
          convertVariables atm =
            let (restr,cond) = translateAtoms
                ...
                output = [ ([ (tvar,case Map.lookup tvar inmp of
                ...
                input = cond
            in return BuchiVariables { outputConstraints = output, inputConstraints = input, atoms = atm }
      in translateGBA $ runIdentity $
         gtlToBuchi convertVariables (gtlModelContract mdl)
