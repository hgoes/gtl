...
removeDeadlocks ba
  = let (rem,keep) = Map.partition List.null (baTransitions ba)
        loop rem mp
          | Map.null rem = mp
          | otherwise = let (rem',mp') = Map.mapEither (\trans -> let ntrans = List.filter (\(_,st) -> not $ Map.member st rem) trans
                                                                  in if List.null ntrans
                                                                     then Left ntrans
                                                                     else Right ntrans
                                                       ) mp
                        in loop rem' mp'
